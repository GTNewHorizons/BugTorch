 ...
 * software to the public domain worldwide. This software is distributed without any warranty. See
 ...
 * <a href="http://xoshiro.di.unimi.it/xoshiro256starstar.c">original C code</a>, which is public domain. This Java port
 * is similarly dedicated to the public domain.
 * <p>
 * Individual instances are not thread-safe. Each thread must have its own instance which is not shared.
 *
...
public class RandomXoshiro256StarStar extends Random {
    ...
    public void nextBytes(byte[] buf, int ofs, int len) {
        ...
        if (ofs + len > buf.length) throw new ArrayIndexOutOfBoundsException(
                "Length " + len + " with offset " + ofs + " is past end of buffer");
        int j = 8;
        long l = 0;
    ...
    /*
     * This is xoshiro256** 1.0, our all-purpose, rock-solid generator. It has excellent (sub-ns) speed, a state (256
     * bits) that is large enough for any parallel application, and it passes all tests we are aware of. For generating
     * just floating-point numbers, xoshiro256+ is even faster. The state must be seeded so that it is not everywhere
     * zero. If you have a 64-bit seed, we suggest to seed a splitmix64 generator and use its output to fill s.
     */
