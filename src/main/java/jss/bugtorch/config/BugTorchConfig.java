...
public class BugTorchConfig {
    ...
    public static void loadBaseConfig(File configFile) {
        ...
        // Backports
        enableFloatingTrapDoors = config.getBoolean(
                "freeFloatingTrapDoors",
                categoryBackport,
                true,
                "Trapdoors no longer require attachment blocks.\nFrom MC 1.9");

        // Bugfixes
        fixBlockSounds = config.getBoolean(
                "fixBlockSounds",
                categoryBugfixes,
                true,
                "Assigns the correct sound types to some blocks.");
        fixJackOLanternBlocksRandomlyTicking = config.getBoolean(
                "fixJackOLanternBlocksRandomlyTicking",
                categoryBugfixes,
                true,
                "Jack O' Lantern blocks will no longer randomly tick.");
        fixPumpkinBlocksRandomlyTicking = config.getBoolean(
                "fixPumpkinBlocksRandomlyTicking",
                categoryBugfixes,
                true,
                "Pumpkin blocks will no longer randomly tick.");
        fixSnowBlocksRandomlyTicking = config.getBoolean(
                "fixSnowBlocksRandomlyTicking",
                categoryBugfixes,
                true,
        ...
        fixTorchBlocksRandomlyTicking = config.getBoolean(
                "fixTorchBlocksRandomlyTicking",
                categoryBugfixes,
                true,
                "Torch blocks will no longer randomly tick.");

        removeBroadcastSettingsButton = config.getBoolean(
                "removeBroadcastSettingsButton",
                categoryTweaks,
                false,
                "Remove the Broadcast Settings button from the options menu.");

        // Features
        addSquidsSounds = config
                .getBoolean("fixSquidsSounds", categoryAdditions, true, "Squids will make sounds (requires TX Loader)");
        addTossAnimation = config.getBoolean(
                "addPlayerTossAnimation",
                categoryAdditions,
                true,
                "Adds Player item toss animation and sound(requires TX Loader)");
        addHitSound = config.getBoolean(
                "addPlayerHitSounds",
                categoryAdditions,
                true,
                "Adds Player attack sounds (requires TX Loader)");

        fixThunderSoundVolume = config
                .getBoolean("fixThunderVolume", categoryBugfixes, true, "Sets thunder sound AttenuationType to LINEAR");
        thunderSoundVolumeMultiplier = config.getFloat(
                "thunderVolumeMultiplier",
                categoryBugfixes,
                1,
                Float.MIN_VALUE,
                Float.MAX_VALUE,
                "Thunder Volume modifier. if fixThunderVolume is enabled");

        mobSaySoundDelay = config.getInt(
                "mobSaySoundDelay",
                categoryTweaks,
                0,
                0,
                Integer.MAX_VALUE,
                "Adds a delay to sounds made by mobs, such as zombie growls etc. set 0 to disable");
    ...
    public static void loadModdedConfig(File configFile) {
        ...
        // Bugfixes
        fixExtraUtilitiesBlockSounds = config.getBoolean(
                "fixExtraUtilitiesBlockSounds",
                categoryBugfixes,
                true,
                "Assigns the correct sound types to some blocks from Extra Utilities.");
        fixPamsTemperatePlantsBlockSounds = config.getBoolean(
                "fixPamsTemperatePlantsBlockSounds",
                categoryBugfixes,
                true,
                "Assigns the correct sound types to some blocks from Pam's Temperate Plants.");
        fixWitcheryBlockSounds = config.getBoolean(
                "fixWitcheryBlockSounds",
                categoryBugfixes,
                true,
                "Assigns the correct sound types to some blocks from Witchery.");

        // Ore dictionary
        registerThaumcraftLeavesToTheOreDictionary = config.getBoolean(
                "registerThaumcraftLeavesToTheOreDictionary",
                categoryOreDictionary,
                true,
                "Register Thaumcraft Greatwood and Silverwood leaves as treeLeaves.");
        registerThaumcraftThaumiumBlockToTheOreDictionary = config.getBoolean(
                "registerThaumcraftThaumiumBlockToTheOreDictionary",
                categoryOreDictionary,
                true,
                "Register Thaumcraft Thaumium Blocks as blockThaumium.");
        registerThaumcraftWoodStairsToTheOreDictionary = config.getBoolean(
                "registerThaumcraftWoodStairsToTheOreDictionary",
                categoryOreDictionary,
                true,
                "Register Thaumcraft Greatwood and Silverwood stairs as stairWood.");
        registerWitcheryWoodSlabsToTheOreDictionary = config.getBoolean(
                "registerWitcheryWoodSlabsToTheOreDictionary",
                categoryOreDictionary,
                true,
                "Register Witchery wooden slabs as slabWood.");

        // Tweaks
        craftThaumcraftAncientStoneSlabsAndStairs = config.getBoolean(
                "craftThaumcraftAncientStoneSlabs",
                categoryTweaks,
                true,
                "Craft Thaumcraft Ancient Stone slabs and stairs.");
        enableVillageNamesMetadataSensitiveTrades = config.getBoolean(
                "enableVillageNamesMetadataSensitiveTrades",
                categoryTweaks,
                true,
                "Enables metadata sensitive trades in Village Names.\nDoes nothing if fixVillagerTradeMetadataDetection in \"bugtorch\\mixins.cfg\" is not enabled.")
                && fixVillagerTradeMetadataDetection;
        reverseCraftThaumcraftSlabs = config.getBoolean(
    ...
    public static void loadBaseMixinConfig(File configFile) {
        ...
        fixMineshaftAirPockets = config.getBoolean(
                "fixMineshaftAirPockets",
                categoryBugfixes,
                true,
        ...
        fixNettyConnectionFailureResourceLeak = config.getBoolean(
                "fixNettyConnectionFailureResourceLeak",
                categoryBugfixes,
                true,
                "Fixes improperly terminated client connections sometimes causing a severe resource leak.");
        fixParticleDepthSorting = config.getBoolean(
                "fixParticleDepthSorting",
                categoryBugfixes,
                true,
                "Fixes particle depth being incorrectly calculated.");
        fixPumpkinPlacementCheck = config.getBoolean(
                "fixPumpkinPlacementCheck",
                categoryBackport,
                true,
                "Pumpkins and Jack O' Lanterns can be placed without a solid block below them.\nFrom MC 1.13, fixes MC-1947");
        fixRedstoneTorchMemoryLeak = config.getBoolean(
                "fixRedstoneTorchMemoryLeak",
                categoryBackport,
                true,
                "Stops Redstone Torches from causing a memory leak by making them use a weak hash map to store burnt out torches.\nFixes MC-101233");
        fixShearedGrassDropDupe = config.getBoolean(
                "fixShearedGrassDropDupe",
                categoryBugfixes,
                true,
                "Shearing tall grass will not give drops in addition to itself.");
        fixShearedLeavesDropDupe = config.getBoolean(
                "fixShearedLeavesDropDupe",
                categoryBugfixes,
                true,
                "Shearing leaves will not give drops in addition to itself.");
        fixShearsNotTakingDamageFromNormalBlocks = config.getBoolean(
                "fixShearsNotTakingDamageFromNormalBlocks",
                categoryBugfixes,
                true,
        ...
        fixSignPacketChatMessages = config.getBoolean(
                "fixSignPacketChatMessages",
                categoryBugfixes,
                true,
                "Sign update packets for signs in unloaded chunks will not send chat messages.\nFrom MC 1.9, fixes MC-3564");
        fixStoneMonsterEggDoubleSpawns = config.getBoolean(
                "fixStoneMonsterEggDoubleSpawns",
                categoryBugfixes,
                true,
                "Stone Monster Eggs only spawn one Silverfish when broken.\nFrom MC 1.8, fixes MC-31081");
        fixStructureComponentDownfillReplacement = config.getBoolean(
                "fixStructureComponentDownfillReplacement",
                categoryBugfixes,
                true,
        ...
        // Tweaks
        enchantmentParticlesForPowerAboveZero = config.getBoolean(
                "enchantmentParticlesForPowerAboveZero",
                categoryTweaks,
                true,
                "Makes Enchantment Tables emit particles for any block with enchantment power.");
        farmlandHydroponics = config
                .getBoolean("farmlandHydroponics", categoryTweaks, false, "Farmland can use hydroponics.");
        farmlandNewTextures = config.getBoolean(
                "farmlandNewTextures",
                categoryTweaks,
                false,
                "New side textures for both wet and dry farmland.");
        farmlandNoTrample = config
                .getBoolean("farmlandNoTrample", categoryTweaks, false, "Farmland will no longer get trampled.");
        lanPortOverride = config.getBoolean(
                "lanPortOverride",
                categoryTweaks,
                false,
                "Override the port used when opening singleplayer to LAN.");
        placeEndPortalsAnywhere = config.getBoolean(
                "placeEndPortalsAnywhere",
                categoryTweaks,
                false,
                "Place End Portals outside of the overworld without them getting removed.");
        placePressurePlatesOnAnyWallOrFence = config.getBoolean(
                "placePressurePlatesOnAnyWallOrFence",
                categoryTweaks,
                true,
                "Place pressure plates on almost any wall or fence.");
        placeTorchesOnAnyFence = config
                .getBoolean("placeTorchesOnAnyFence", categoryTweaks, true, "Place torches on almost any fence.");
        placeTorchesOnAnyWall = config
                .getBoolean("placeTorchesOnAnyWall", categoryTweaks, true, "Place torches on almost any wall.");
        potionParticlesAreClearForClientPlayer = config.getBoolean(
                "potionParticlesAreClearForClientPlayer",
                categoryTweaks,
                false,
                "Potion particles coming off of the player entity you control are always clear.");
        reduceLightningVolume = config.getFloat(
                "reduceLightningVolume",
                categoryTweaks,
                10000f,
                2f,
                10000f,
                "Reduces lightning volume and effective range.\nSet to 10,000 to disable.");
        removeEntityDuplicateExtendedPropertiesIdentifierSpam = config.getBoolean(
        ...
        scaledDrowningDamageMaxHealthFlat = config.getFloat(
                "scaledDrowningDamageMaxHealthFlat",
                categoryTweaks,
                0f,
                0f,
                20000f,
                "Amount of flat player health to remove each drowning tick.\nSet to 0 to disable.");
        scaledDrowningDamageMaxHealthMult = config.getFloat(
                "scaledDrowningDamageMaxHealthMult",
                categoryTweaks,
                0f,
                0f,
                1f,
                "Portion of max player health to remove each drowning tick.\nSet to 0 to disable.");
        scaledFireDamageMaxHealthFlat = config.getFloat(
                "scaledFireDamageMaxHealthFlat",
                categoryTweaks,
                0f,
                0f,
                20000f,
                "Amount of flat player health to remove each fire tick.\nSet to 0 to disable.");
        scaledFireDamageMaxHealthMult = config.getFloat(
                "scaledFireDamageMaxHealthMult",
                categoryTweaks,
                0f,
                0f,
                1f,
                "Portion of max player health to remove each fire tick.\nSet to 0 to disable.");
        scaledLavaDamageMaxHealthFlat = config.getFloat(
                "scaledLavaDamageMaxHealthFlat",
                categoryTweaks,
                0f,
                0f,
                20000f,
                "Amount of flat player health to remove each lava tick.\nSet to 0 to disable.");
        scaledLavaDamageMaxHealthMult = config.getFloat(
                "scaledLavaDamageMaxHealthMult",
                categoryTweaks,
                0f,
                0f,
                1f,
                "Portion of max player health to remove each lava tick.\nSet to 0 to disable.");
        scaledPoisonDamageMaxHealthFlat = config.getFloat(
                "scaledPoisonDamageMaxHealthFlat",
                categoryTweaks,
                0f,
                0f,
                20000f,
                "Portion of max player health to remove each poison effect tick.\nSet to 0 to disable.");
        scaledPoisonDamageMaxHealthMult = config.getFloat(
                "scaledPoisonDamageMaxHealthMult",
                categoryTweaks,
                0f,
                0f,
                1f,
                "Portion of max player health to remove each poison effect tick.\nSet to 0 to disable.");
        scaledStarvationDamageMaxHealthFlat = config.getFloat(
                "scaledStarvationDamageMaxHealthFlat",
                categoryTweaks,
                0f,
                0f,
                20000f,
                "Amount of flat player health to remove each starvation tick.\nSet to 0 to disable.");
        scaledStarvationDamageMaxHealthMult = config.getFloat(
                "scaledStarvationDamageMaxHealthMult",
                categoryTweaks,
                0f,
                0f,
                1f,
                "Portion of max player health to remove each starvation tick.\nSet to 0 to disable.");
        scaledSuffocationDamageMaxHealthFlat = config.getFloat(
                "scaledSuffocationDamageMaxHealthFlat",
                categoryTweaks,
                0f,
                0f,
                20000f,
                "Portion of max player health to remove each suffocation tick.\nSet to 0 to disable.");
        scaledSuffocationDamageMaxHealthMult = config.getFloat(
                "scaledSuffocationDamageMaxHealthMult",
                categoryTweaks,
                0f,
                0f,
                1f,
                "Portion of max player health to remove each suffocation tick.\nSet to 0 to disable.");
        scaledWitherDamageMaxHealthFlat = config.getFloat(
                "scaledWitherDamageMaxHealthFlat",
                categoryTweaks,
                0f,
                0f,
                20000f,
                "Portion of max player health to remove each wither effect tick.\nSet to 0 to disable.");
        scaledWitherDamageMaxHealthMult = config.getFloat(
                "scaledWitherDamageMaxHealthMult",
                categoryTweaks,
                0f,
                0f,
                1f,
        ...
        lanPortToUseForOverride = config.getInt(
                "lanPortToUseForOverride",
                categoryTweaks,
                25565,
                1024,
                49151,
        ...
        if (config.hasKey(categoryBackport, "throwEnderPearlsInCrativeMode")) {
            throwEnderPearlsInCreativeMode = config
                    .getBoolean("throwEnderPearlsInCrativeMode", categoryBackport, true, "");
            config.getCategory(categoryBackport).get("throwEnderPearlsInCrativeMode")
                    .set(throwEnderPearlsInCreativeMode);
        ...
        if (config.hasKey(categoryBugfixes, "fixShearedBlocksDropExtraItems")) {
            boolean fixShearedBlocksDropExtraItems = config
                    .getBoolean("fixShearedBlocksDropExtraItems", categoryBackport, true, "");
    ...
    public static void loadModdedMixinConfig(File configFile) {
        ...
        // Bugfixes
        fixCrayfishFurnitureNullPointerException = config.getBoolean(
                "fixCrayfishFurnitureNullPointerExceptions",
                categoryBugfixes,
                true,
                "Makes several TEs from MrCrayfish's Furniture Mod implement ISidedInventory correctly.");
        fixGanysSurfaceOpenTrapdoorBackTexture = config.getBoolean(
                "fixGanysSurfaceOpenTrapdoorBackTexture",
                categoryBugfixes,
                true,
                "Makes Gany's Surface trapdoors use the correct back texture when open.");
        fixThaumcraftCandleColorArrayOutOfBounds = config.getBoolean(
                "fixThaumcraftCandleColorArrayOutOfBounds",
                categoryBugfixes,
                true,
                "Makes Thaumcraft candles not cause an array out of bounds exception if rendered with metadata greater than 15.");
        fixWitcheryGarlicGarlandBlockBounds = config.getBoolean(
                "fixWitcheryGarlicGarlandBlockBounds",
                categoryBugfixes,
                true,
                "Makes Witchery Garlic Garlands use correct block bounds on every rotation.");
        fixWitcheryLeavesOptifineRendering = config.getBoolean(
                "fixWitcheryLeavesOptifineRendering",
                categoryBugfixes,
                true,
                "Makes Witchery Leaves respect Optifine render settings.");
        fixWitcheryLeavesShearDupe = config.getBoolean(
                "fixWitcheryLeavesShearDupe",
                categoryBugfixes,
                true,
                "Partially fixes a Forge shearing bug that impacts Witchery Leaves.");
        reuseAetherIIRenderPlayer = config.getBoolean(
                "reuseAetherIIRenderPlayer",
                categoryPerformance,
                true,
                "Makes Aether II reuse the same player renderer object across frames.");

        // Tweaks
        disableCrayfishFurnitureAchievements = config.getBoolean(
                "disableCrayfishFurnitureAchievements",
                categoryTweaks,
                false,
                "Disables MrCrayfish's Furniture Mod achievements.");
        proxyLLibraryPastebin = config.getBoolean(
                "proxyLLibraryPastebin",
                categoryTweaks,
                false,
        ...
        if (config.hasKey(categoryBugfixes, "fixWitcheryLeavesShearDupeAndOptifineRendering")) {
            boolean leafFix = config
                    .getBoolean("fixWitcheryLeavesShearDupeAndOptifineRendering", categoryBugfixes, true, "");
