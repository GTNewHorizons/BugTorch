...
public class BroadcastSettingsRemover {
    ...
    public void onGui(GuiScreenEvent.InitGuiEvent.Post event) {
        if (event.gui instanceof GuiOptions) {
            GuiButton superSecretSettings = ((List<GuiButton>) event.buttonList).stream()
                    .filter(button -> button.id == 8675309).findFirst().get();
            GuiButton broadcastSettings = ((List<GuiButton>) event.buttonList).stream()
                    .filter(button -> button.id == 107).findFirst().get();
