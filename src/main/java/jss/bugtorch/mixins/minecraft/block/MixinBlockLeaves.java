    ...
    public void updateTick(World world, int x, int y, int z, Random random) {
        if (!world.isRemote) {
            ...
            if ((meta & 8) != 0 && (meta & 4) == 0) {
                            ...
                            for (searchZ = -decayRange; searchZ <= decayRange; ++searchZ) {
                                ...
                                if (!block.canSustainLeaves(world, x + searchX, y + searchY, z + searchZ)) {
                                    if (block.isLeaves(world, x + searchX, y + searchY, z + searchZ)) {
                                        field_150128_a[
                                                (searchX + halfSearchRangeMax) * SearchRangeSquared
                        ...
                        for (searchY = -decayRange; searchY <= decayRange; ++searchY) {
                            for (searchZ = -decayRange; searchZ <= decayRange; ++searchZ) {
                                for (int i = -decayRange; i <= decayRange; ++i) {
                                    if (field_150128_a[
                                                    (searchY + halfSearchRangeMax) * SearchRangeSquared
                                                            + (searchZ + halfSearchRangeMax) * searchRangeMax
                                                            + i
                                                            + halfSearchRangeMax]
                                            == searchX - 1) {
                                        if (field_150128_a[
                                                        (searchY + halfSearchRangeMax - 1) * SearchRangeSquared
                                                                + (searchZ + halfSearchRangeMax) * searchRangeMax
                                                                + i
                                                                + halfSearchRangeMax]
                                                == -2) {
                ...
                searchX = field_150128_a[
                        halfSearchRangeMax * SearchRangeSquared
                                + halfSearchRangeMax * searchRangeMax
                                + halfSearchRangeMax];
