                            ...
                            for (searchZ = -decayRange; searchZ <= decayRange; ++searchZ) {
                                ...
                                if (!block.canSustainLeaves(world, x + searchX, y + searchY, z + searchZ)) {
                                    if (block.isLeaves(world, x + searchX, y + searchY, z + searchZ)) {
                                        field_150128_a[(searchX + halfSearchRangeMax) * SearchRangeSquared
                                                + (searchY + halfSearchRangeMax) * searchRangeMax
                                                + searchZ
                                                + halfSearchRangeMax] = -2;
                                    } else {
                                        field_150128_a[(searchX + halfSearchRangeMax) * SearchRangeSquared
                                                + (searchY + halfSearchRangeMax) * searchRangeMax
                                                + searchZ
                                                + halfSearchRangeMax] = -1;
                                    }
                                } else {
                                    field_150128_a[(searchX + halfSearchRangeMax) * SearchRangeSquared
                                            + (searchY + halfSearchRangeMax) * searchRangeMax
                                            + searchZ
                                            + halfSearchRangeMax] = 0;
                                }
                            }
                        }
                    }
                            ...
                            for (searchZ = -decayRange; searchZ <= decayRange; ++searchZ) {
                                for (int i = -decayRange; i <= decayRange; ++i) {
                                    if (field_150128_a[(searchY + halfSearchRangeMax) * SearchRangeSquared
                                        ...
                                        if (field_150128_a[(searchY + halfSearchRangeMax - 1) * SearchRangeSquared
                                                + (searchZ + halfSearchRangeMax) * searchRangeMax
                                                + i
                                                + halfSearchRangeMax] == -2) {
                                            field_150128_a[(searchY + halfSearchRangeMax - 1) * SearchRangeSquared
                                                    + (searchZ + halfSearchRangeMax) * searchRangeMax
                                                    + i
                                                    + halfSearchRangeMax] = searchX;
                                        }
                                        ...
                                        if (field_150128_a[(searchY + halfSearchRangeMax) * SearchRangeSquared
                                                ...
                                                + i
                                                + halfSearchRangeMax] == -2) {
                                            field_150128_a[(searchY + halfSearchRangeMax) * SearchRangeSquared
                                                    + (searchZ + halfSearchRangeMax - 1) * searchRangeMax
                                                    + i
                                                    + halfSearchRangeMax] = searchX;
                                        }
                                        ...
                                        if (field_150128_a[(searchY + halfSearchRangeMax) * SearchRangeSquared
                                                + (searchZ + halfSearchRangeMax) * searchRangeMax
                                                + (i + halfSearchRangeMax - 1)] == -2) {
                                            field_150128_a[(searchY + halfSearchRangeMax) * SearchRangeSquared
                                                    + (searchZ + halfSearchRangeMax) * searchRangeMax
                                                    + (i + halfSearchRangeMax - 1)] = searchX;
                                        }
